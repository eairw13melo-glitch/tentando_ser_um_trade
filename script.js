...
//   Trader Toolkit - Totalmente limpo e organizado
//   Removida seção Evolução do Patrimônio
// ======================================================
...
// Lista completa de IDs dos checkboxes (checklists)
...
function addStop() {
    if (dailyStopCount >= 2) {
        alert("Limite de 2 stops diários atingido!");
        return;
    }
    dailyStopCount++;
    updateStopDisplay();
    enforceStopLimitUI();
    alert(dailyStopCount === 2 
        ? "🔴 DOIS STOPS: Fim do dia de trading." 
        : "⚠️ Primeiro stop. Pausa de 15 minutos.");
}
...
function updatePatrimonioDisplay() {
    ...
    if (el) el.innerText = `R$ ${patrimonio.toFixed(2)}`;
    localStorage.setItem('trader_patrimonio', patrimonio);
}

function editarPatrimonio() {
    const novo = prompt("Novo patrimônio (R$):", patrimonio.toFixed(2));
...
function addAsset() {
    ...
    if (novo && !ativosList.includes(novo)) {
        ...
        alert("Ativo já existe ou é inválido.");
    }
}
...
function editTradeById(id) {
    ...
    if (confirm("Editar trade? Após alterar os campos, clique em 'Adicionar Trade'.")) {
        deleteTrade(id);
    }
}
...
function getFormTrade() {
    ...
    if (!data || !ativo || isNaN(entrada)) {
        alert("❌ Preencha Data, Ativo e Preço de Entrada.");
        return null;
    }
...
function renderTrades() {
    ...
    if (trades.length === 0) {
        container.innerHTML = '<div class="empty-message">Nenhum trade registrado ainda.</div>';
        return;
    }

    let html = '';
    trades.forEach(t => {
        const resultClass = (t.resultado && parseFloat(t.resultado) > 0) 
            ? 'style="color:#1e7e5e;"' 
            : (t.resultado && parseFloat(t.resultado) < 0) 
            ? 'style="color:#b13e3e;"' 
            : '';
        ...
        html += `
            <div class="trade-item" data-id="${t.id}">
                <div class="trade-info">
                    ...
                    Risco: ${t.riscoPercent ? t.riscoPercent + '%' : '-'} | 
                    Resultado: <span ${resultClass}>${t.resultado || 'pendente'}</span><br>
    ...
    document.querySelectorAll('.btn-edit').forEach(btn => {
        btn.addEventListener('click', () => editTradeById(btn.getAttribute('data-id')));
    });
    document.querySelectorAll('.btn-delete').forEach(btn => {
        btn.addEventListener('click', () => {
            if (confirm("Excluir este trade permanentemente?")) {
...
function exportToCSV() {
    if (trades.length === 0) {
        alert("Sem trades para exportar.");
        return;
    }

    const headers = ['Data','Ativo','Tipo','Entrada','Saída','Stop','Take','Risco%','Estratégia','Gestão','Resultado','Notas'];
    const rows = trades.map(t => [
        t.data, t.ativo, t.tipo, t.entrada, t.saida ?? '', 
        t.stopLoss ?? '', t.takeProfit ?? '', t.riscoPercent ?? '', 
        t.estrategia ?? '', t.riskRule ?? '', t.resultado ?? '', t.notas ?? ''
    ]);
    ...
    a.href = url;
    a.download = `diario_trades_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
...
// ==================== MONITOR DE AÇÕES (YAHOO FINANCE) ====================
const stocks = [
...
async function updateStocks() {
    ...
    grid.innerHTML = '<div class="loading">Carregando cotações em tempo real...</div>';
    ...
    results.forEach(stock => {
        ...
        const sign = stock.quote && stock.quote.change >= 0 ? '+' : '';

        card.innerHTML = `
            <div class="stock-symbol">${stock.name}</div>
            <div class="stock-price">${stock.quote ? `R$ ${stock.quote.price.toFixed(2)}` : '--'}</div>
            <div class="stock-change ${changeClass}">
                ${stock.quote ? `${sign}${stock.quote.change.toFixed(2)} (${sign}${stock.quote.changePercent.toFixed(2)}%)` : '--'}
            </div>
        `;
...
function startStockUpdater() {
    ...
    stockInterval = setInterval(updateStocks, 60000); // 1 minuto
}
...
function initAccordion() {
    ...
    headers.forEach(header => {
        header.addEventListener('click', () => {
            ...
            if (icon) {
                icon.style.transform = body.classList.contains('active') ? 'rotate(0deg)' : 'rotate(-90deg)';
            }
        });
...
function displayCurrentDate() {
    ...
    if (span) {
        span.innerText = new Date().toLocaleDateString('pt-BR', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
